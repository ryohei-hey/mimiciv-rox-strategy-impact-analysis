...
# Dont forget elig_1==Patiets ICU admisson within 7days
...
### Censoring function for time-varying ROX threshold strategy
### Simple version: ROX < cutoff → intubate
arm1.censor.grace <- function(rox, treat_cum, treat, highflow_lag, cal_time,
  ...
  # ROX < cutoffとなった最初の時点を記録（grace period用）
  # ※一度記録されたらリセットしない
  ...
  for (i in 1:n) {
    ...
    if (!is.na(treat_cum_lag[i]) && treat_cum_lag[i] == 0 &&
      ...
      if (current_rox < cutoff) {
        ...
        # ★ 修正点: ROXが回復してもカウンターをリセットしない
        # （以前はここで first_below_cutoff <- NA としていた）

        # Grace期間中にROXが回復しても、カウンターは継続
        # Grace期間を過ぎていれば、挿管していなくても遵守（ROX回復したため）
        # → ただし、一度閾値を下回った事実は記録し続ける

        if (current_treat == 1) {
          # ROX >= cutoff なのに挿管した場合
          # ただし、grace期間中で以前にROX < cutoffだった場合は遵守とみなす
          if (grace_hours > 0 && !is.na(first_below_cutoff) &&
              i < first_below_cutoff + grace_hours) {
            # Grace期間内で、以前にROX < cutoffだった → 挿管は遵守
            # （何もしない、ループ継続）
          } else if (grace_hours > 0 && !is.na(first_below_cutoff)) {
            # Grace期間は過ぎているが、以前にROX < cutoffだった
            # ROXが回復した状態での挿管 → 非遵守
            first.cens <- i
            break
          } else {
            # Grace periodなし、または一度もROX < cutoffになっていない
            first.cens <- i
            break
          }
        }
...
clone.boot.graph <- function(data, indices, grace_hours = 1){
  ...
  # SENSITIVITY ANALYSIS: Removed resp_rate, spo2, fio2 from the model
...
# Grace period設定
GRACE_HOURS <- 1  # ここを変更することで感度分析が可能（0, 1, 2, 3など）

# 並列処理準備
cl <- makeCluster(use_cores)
registerDoParallel(cl)

clusterExport(cl, c("dt", "dt_ids", "arm1.censor.grace", "arm1.grace.flag", "clone.boot.graph", "GRACE_HOURS"))
clusterEvalQ(cl, {
  library(data.table)
  library(dplyr)
})

cat("\n=== 感度分析: resp_rate, spo2, fio2除外 ===\n")
cat("=== 本番ブートストラップ（R=500, GRACE PERIOD=", GRACE_HOURS, "時間） ===\n")
cat("累積発生率曲線用\n")
...
# 保存
index<-"complex"
output_filename <- paste0("outputs/ROX/bootdata/bootstrap_graphs_",index,"_r500_grace", GRACE_HOURS, "hr_sensitivity_no_resp_spo2_fio2.rds")
...
plot.ipw <- ggplot(graph, aes(x = time_days)) +
  ...
  scale_color_manual(
    ...
    labels = c("Usual Care" = "Usual Care",
               "ROX Strategy" = paste0("ROX-guided intubation (Complex strategy", ", ", GRACE_HOURS, " hr grace)"))
  ) +

  labs(subtitle = "Sensitivity Analysis: Excluding resp_rate, spo2, fio2 from adjustment models")

print(plot.ipw)

plot_filename <- paste0("outputs/ROX/Fig.cumulative_incidence_r500_",index,"_30d_with_CI_grace", GRACE_HOURS, "hr_sensitivity_no_resp_spo2_fio2.png")
...
ft_paper <- flextable(complete_results) %>%
  ...
  add_footer_lines(
    values = c(
      ...
      "SENSITIVITY ANALYSIS: resp_rate, spo2, fio2 excluded from adjustment models"
    )
  ) %>%
...
doc_paper <- doc_paper %>%
  body_add_par(paste0("Table 1. Cumulative Mortality Outcomes at 7, 14, and 30 Days (Grace Period: ", GRACE_HOURS, " hour)"),
               style = "heading 1") %>%
  body_add_par("Sensitivity Analysis: Excluding resp_rate, spo2, fio2 from adjustment models",
               style = "heading 2") %>%
  body_add_flextable(ft_paper)

doc_filename <- paste0("outputs/ROX/mortality_results_7_14_30day_",index,"_grace", GRACE_HOURS, "hr_sensitivity_no_resp_spo2_fio2.docx")
