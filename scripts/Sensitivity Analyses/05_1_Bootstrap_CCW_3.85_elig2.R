...
dt |> filter(elig_1==1) |> dplyr::select(id)|> unique()|> nrow()
dt |> filter(elig_2==1) |> dplyr::select(id)|> unique()|> nrow()
...
# 保存
index_label <- paste0(index, "_elig2")
...
plot.ipw <- ggplot(graph, aes(x = time_days)) +
  ...
  scale_color_manual(
    ...
    labels = c("Usual Care" ="Usual Care",
               "ROX Strategy" = paste0("ROX-guided intubation (ROX < ",index, ", elig_2, ", GRACE_HOURS, " hr grace)"))
  )
