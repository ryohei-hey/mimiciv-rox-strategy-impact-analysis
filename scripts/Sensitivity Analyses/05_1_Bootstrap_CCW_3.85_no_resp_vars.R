...
# Sensitivity analysis: Weight model without resp_rate, spo2, fio2
# (ROX index components excluded to avoid collider bias)
...
clone.boot.graph <- function(data, indices, grace_hours = 1){
  ...
  # NOTE: resp_rate, spo2, fio2 removed for sensitivity analysis
  psc.denom.a <- suppressWarnings(glm(
    treat ~ age + female + medicare_medicaid + language_grouped + Married +
      ...
      # resp_rate + spo2 + fio2 +  # REMOVED for sensitivity analysis
...
cat("\n=== 本番ブートストラップ（R=500, GRACE PERIOD=", GRACE_HOURS, "時間, resp_rate/spo2/fio2除外） ===\n")
cat("累積発生率曲線用\n")
...
# 保存
index_label <- paste0(index, "_no_resp_vars")
output_filename <- paste0("outputs/ROX/bootdata/bootstrap_graphs_",index_label,"_r500_grace", GRACE_HOURS, "hr.rds")
...
plot.ipw <- ggplot(graph, aes(x = time_days)) +
  ...
  scale_color_manual(
    ...
    labels = c("Usual Care" ="Usual Care",
               "ROX Strategy" = paste0("ROX-guided intubation (ROX < ",index, ", ", GRACE_HOURS, " hr grace, no resp vars)"))
  )

print(plot.ipw)

plot_filename <- paste0("outputs/ROX/Fig.cumulative_incidence_r500_",index_label,"_30d_with_CI_grace", GRACE_HOURS, "hr.png")
...
doc_paper <- doc_paper %>%
  body_add_par(paste0("Table. Cumulative Mortality Outcomes at 7, 14, and 30 Days (Sensitivity Analysis: Weight model without resp_rate/spo2/fio2, Grace Period: ", GRACE_HOURS, " hour)"),
               style = "heading 1") %>%
  body_add_flextable(ft_paper)

doc_filename <- paste0("outputs/ROX/mortality_results_7_14_30day_",index_label,"_grace", GRACE_HOURS, "hr.docx")
