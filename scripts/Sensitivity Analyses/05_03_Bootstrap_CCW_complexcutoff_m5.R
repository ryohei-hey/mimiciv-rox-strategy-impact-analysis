# ============================================================================
# Bootstrap Analysis for Time-Varying ROX Strategy with Multiple Imputation (m=5)
# ============================================================================
...
# Grace period: 1 hour
# Strategy: Time-varying ROX threshold (2.85/3.47/3.85 based on time from HFNC start)
#   - 1-5 hours: ROX < 2.85
#   - 6-11 hours: ROX < 3.47
#   - 12+ hours: ROX < 3.85
# ============================================================================
...
cat("\n============================================================================\n")
cat("  Bootstrap Analysis: Time-Varying ROX Strategy (Multiple Imputation m=5)\n")
cat("============================================================================\n")
cat("ROX Thresholds:\n")
cat("  - 1-5 hours from HFNC start: ROX < 2.85\n")
cat("  - 6-11 hours from HFNC start: ROX < 3.47\n")
cat("  - 12+ hours from HFNC start: ROX < 3.85\n")
...
# Censoring function for time-varying ROX threshold strategy WITH GRACE PERIOD -----
arm1.censor.grace <- function(rox, treat_cum, treat, highflow_lag, cal_time,
                              grace_hours = 1) {
  n <- length(rox)
  my.censor <- rep(0, n)
  first.cens <- NA

  # Create lagged treat_cum to check if before intubation
  treat_cum_lag <- c(0, treat_cum[1:(n-1)])

  # Function to get time-varying cutoff based on hours from HFNC start
  get_cutoff <- function(time_hr) {
    if (time_hr >= 1 && time_hr <= 5) {
      ...
      return(3.85)  # 12 hours and later
    }
  }

  # Record first time point when ROX < cutoff (for grace period)
  # Once recorded, not reset
  ...
  for (i in 1:n) {
    ...
    if (!is.na(treat_cum_lag[i]) && treat_cum_lag[i] == 0 &&
      ...
      # === ROX < cutoff: Should intubate ===
      if (current_rox < cutoff) {

        # Grace period handling
        if (grace_hours > 0) {
          # Record first time point when ROX < cutoff
          if (is.na(first_below_cutoff)) {
            first_below_cutoff <- i
          }

          # Non-adherence if not intubated after grace period
          if (i >= first_below_cutoff + grace_hours && current_treat == 0) {
            first.cens <- i
            break
          }
        } else {
          # No grace period: immediate assessment
          if (current_treat == 0) {
            first.cens <- i
            break
          }
        ...
        # === ROX >= cutoff: Should continue HFNC ===

        # IMPORTANT: Counter is NOT reset even if ROX recovers
        # (Previously, first_below_cutoff <- NA was here)

        # Even if ROX recovers during grace period, counter continues
        # If grace period has passed, not intubating is adherent (ROX recovered)
        # → However, the fact that ROX once fell below cutoff is still recorded

        if (current_treat == 1) {
          # Intubated despite ROX >= cutoff
          # However, if within grace period and ROX was previously < cutoff, consider as adherent
          if (grace_hours > 0 && !is.na(first_below_cutoff) &&
              i < first_below_cutoff + grace_hours) {
            # Within grace period, ROX was previously < cutoff → intubation is adherent
            # (Do nothing, continue loop)
          } else if (grace_hours > 0 && !is.na(first_below_cutoff)) {
            # Grace period has passed but ROX was previously < cutoff
            # Intubation with recovered ROX → non-adherent
            first.cens <- i
            break
          } else {
            # No grace period, or ROX never went below cutoff
            first.cens <- i
            break
          }
        }
...
### Grace period flag function (time-varying threshold version)
arm1.grace.flag <- function(rox, treat_cum, highflow_lag, cal_time,
                            grace_hours = 1){
  n <- length(rox)
  grace <- rep(0, n)
  treat_cum_lag <- c(0, treat_cum[1:(n-1)])
...
clone.boot.graph <- function(data, indices, dt_full, grace_hours = 1){
  ...
  arm1.d.c[, my.censor := arm1.censor.grace(rox, treat_cum, treat, highflow_lag,
                                            cal_time, grace_hours = grace_hours), by = bid]
...
for(imp in 1:N_IMPUTATIONS) {
  ...
  # Save results
  output_file <- sprintf("outputs/ROX/bootdata/bootstrap_results_complex_imp%d.rds", imp)
...
cat(sprintf("Results saved in: outputs/ROX/bootdata/bootstrap_results_complex_imp*.rds\n\n"))
...
cat("Next step: Run pooling analysis using Rubin's Rule (05_04_summary script)\n\n")
