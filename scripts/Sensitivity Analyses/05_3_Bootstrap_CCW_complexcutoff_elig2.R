...
# Sensitivity analysis: elig_2==1 (excludes intubation within 1 hour from elig_1)
dt<-dt |> filter(elig_2==1)
...
cat("\n=== 本番ブートストラップ（R=500, GRACE PERIOD=", GRACE_HOURS, "時間, elig_2使用） ===\n")
cat("累積発生率曲線用\n")
...
plot.ipw <- ggplot(graph, aes(x = time_days)) +
  ...
  scale_color_manual(
    ...
    labels = c("Usual Care" = "Usual Care",
               "ROX Strategy" = paste0("ROX-guided intubation (Complex strategy, elig_2", ", ", GRACE_HOURS, " hr grace)"))
  )
...
ft_paper <- flextable(complete_results) %>%
  ...
  add_footer_lines(
    values = c(
      ...
      "Sensitivity analysis: elig_2 (excludes intubation within 1 hour)",
...
doc_paper <- doc_paper %>%
  body_add_par(paste0("Table. Cumulative Mortality Outcomes at 7, 14, and 30 Days (Sensitivity Analysis: elig_2, Grace Period: ", GRACE_HOURS, " hour)"),
               style = "heading 1") %>%
