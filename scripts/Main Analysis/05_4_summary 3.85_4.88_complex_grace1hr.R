...
# File paths based on GRACE_HOURS
file_3.85 <- paste0("outputs/ROX/bootdata/bootstrap_graphs_3.85_r500_grace", GRACE_HOURS, "hr.rds")
file_4.88 <- paste0("outputs/ROX/bootdata/bootstrap_graphs_4.88_r500_grace", GRACE_HOURS, "hr.rds")
file_complex <- paste0("outputs/ROX/bootdata/bootstrap_graphs_complex_r500_grace", GRACE_HOURS, "hr.rds")
...
plot_filename <- paste0("outputs/ROX/Fig.cumulative_incidence_all_strategies_grace", GRACE_HOURS, "hr.png")
...
plot_filename_noCI <- paste0("outputs/ROX/Fig.cumulative_incidence_all_strategies_noCI_grace", GRACE_HOURS, "hr.png")
...
ggsave(paste0("outputs/ROX/Fig_trial1_ROX3.85_grace", GRACE_HOURS, "hr.png"),
       plot_trial1, width = 8, height = 6, dpi = 300)
ggsave(paste0("outputs/ROX/Fig_trial2_ROX4.88_grace", GRACE_HOURS, "hr.png"),
       plot_trial2, width = 8, height = 6, dpi = 300)
ggsave(paste0("outputs/ROX/Fig_trial3_TimeVarying_grace", GRACE_HOURS, "hr.png"),
...
ggsave(paste0("outputs/ROX/Fig_all_trials_combined_grace", GRACE_HOURS, "hr.png"),
       plot_combined_trials, width = 8, height = 16, dpi = 300)

ggsave(paste0("C:/Users/ryohe/Dropbox (個人)/Research/2025ROX_ImpactAnalysis/Manuscript/Table and figures/Figure2_all_trials_combined_grace", GRACE_HOURS, "hr.png"),
       plot_combined_trials, width = 8, height = 16, dpi = 300)

cat("\n✓ Combined trial figure saved:", paste0("outputs/ROX/Fig_all_trials_combined_grace", GRACE_HOURS, "hr.png"), "\n")
...
doc_filename <- paste0("outputs/ROX/mortality_comparison_all_strategies_grace", GRACE_HOURS, "hr.docx")
print(doc_comparison, target = doc_filename)

cat("\n✓ Comparison table saved:", doc_filename, "\n")


## Step 5: Table X for manuscripts ------
# Table X. Effect of ROX-Guided Strategy vs Usual Care on 30-Day Risk of Mortality (Primary Outcome)
...
# Create flextable for Table X
ft_table_x <- flextable(table_x) %>%
  theme_booktabs() %>%
  autofit() %>%
  align(align = "center", part = "all") %>%
  ...
  add_footer_lines(
    values = c(
      ...
      "†Models were adjusted for time since HFNC initiation (linear and quadratic terms) and the following baseline covariates measured at HFNC initiation: age, sex, insurance status (Medicare or Medicaid versus other), primary language, marital status, race or ethnicity, presence of sepsis according to Sepsis 3 criteria, Elixhauser comorbidity index with van Walraven weighting, and hours from ICU admission to HFNC initiation. We additionally adjusted for the following time-varying covariates measured at each hour: heart rate, mean arterial pressure, temperature, respiratory rate, peripheral oxygen saturation (SpO2), fraction of inspired oxygen (FiO2), Glasgow Coma Scale category, arterial pH, arterial PaO2, arterial PaCO2, Sequential Organ Failure Assessment (SOFA) score within the preceding 24 hours, receipt of continuous renal replacement therapy, receipt of vasopressors, and documented treatment limitations."
    )
  ) %>%
  fontsize(part = "footer", size = 8) %>%
  italic(part = "footer")
...
# Save Table X to Word document


# 保存先のパス
path1 <- paste0("outputs/ROX/S.Tab_ROX_strategy_effect_grace", GRACE_HOURS, "hr.docx")
...
#Since this study was exploratory, no statistical testing was performed, and all results were reported as point estimates with CIs.
...
path1_step6 <- paste0("outputs/ROX/table3_multiple_timepoints_grace", GRACE_HOURS, "hr.docx")
path2_step6 <- paste0("C:/Users/ryohe/Dropbox (個人)/Research/2025ROX_ImpactAnalysis/Manuscript/Table and figures/Table3_ROX_strategy_effect_grace", GRACE_HOURS, "hr.docx")
...
forest_plot <- ggplot(forest_data, aes(y = y_pos)) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = 11, margin = ggplot2::margin(t = 10)),
...
# Save forest plot
forest_path1 <- paste0("outputs/ROX/Fig_forest_plot_grace", GRACE_HOURS, "hr.png")
...
forest_rd_path1 <- paste0("outputs/ROX/Fig_forest_plot_RD_grace", GRACE_HOURS, "hr.png")
...
# Save E-value table
evalue_path <- paste0("outputs/ROX/S.Tab_Evalue_sensitivity_grace", GRACE_HOURS, "hr.docx")
