...
# Sept 21: Create Antibiotic hourly  

## Purpose  
Extract antibiotic administration data from derived_antibiotic table and create
hourly binary variables to join to the person-hourly dataset.

## Approach (same pattern as Step 11: vasopressors)
  
1. Extract antibiotic data for HFNC cohort from derived_antibiotic  
2. Apply 24-hour rule: each administration start covers at least 24h (accounts for dosing intervals)  
3. Expand to hourly binary flags (including IV route distinction)  
4. Join to person_hourly_with_gcs
  
## Output Variables  
- antibiotic_use: 0/1 (any antibiotic in use)  
- antibiotic_iv: 0/1 (IV antibiotic in use)  
- antibiotic_count: number of concurrent antibiotics  
  
```{r}
#| label: step20-0-check-source

cat("\n")
cat("=", rep("=", 60), "\n", sep = "")
cat("Step 20: Antibiotic Use Data Extraction\n")
cat("=", rep("=", 60), "\n", sep = "")

# Check derived_antibiotic table exists
if (!"derived_antibiotic" %in% dbListTables(con)) {
  stop("derived_antibiotic table not found")
}

# Check table structure
abx_columns <- dbGetQuery(con, "
  SELECT column_name, data_type 
  FROM information_schema.columns 
  WHERE table_name = 'derived_antibiotic'
  ORDER BY ordinal_position
")

cat("derived_antibiotic columns:\n")
print(abx_columns)

# Check antibiotic data availability for HFNC cohort
abx_availability <- dbGetQuery(con, "
  SELECT 
      COUNT(*) as total_records,
  ...
  FROM derived_antibiotic
  WHERE stay_id IN (SELECT stay_id FROM hfnc_cohort_final)
")

cat("\nAntibiotic data availability for HFNC cohort:\n")
print(abx_availability)
```


```{r}
#| label: step20-0-check-route
...
```{r}
#| label: step20-1-extract-antibiotics

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.1: Extract antibiotic data\n")
cat("=", rep("=", 50), "\n", sep = "")

# Extract antibiotic data for HFNC cohort
dbExecute(con, "
  CREATE OR REPLACE TABLE abx_base AS
  SELECT 
      abx.subject_id,
      abx.hadm_id,
      abx.stay_id,
      ...
      abx.starttime,
      abx.stoptime,
      -- IV route flag (includes IV, IV DRIP, IVPB, etc.)
      CASE 
          WHEN UPPER(abx.route) LIKE '%IV%' THEN 1
          ELSE 0
      END AS is_iv
  FROM derived_antibiotic abx
  WHERE abx.stay_id IN (SELECT stay_id FROM hfnc_cohort_final)
      AND abx.starttime IS NOT NULL
      AND abx.stoptime IS NOT NULL
      AND abx.starttime <= abx.stoptime
")

cat("  done\n")

# Basic statistics
abx_stats <- dbGetQuery(con, "
  SELECT 
      COUNT(*) as total_events,
      COUNT(DISTINCT stay_id) as unique_patients,
      COUNT(DISTINCT antibiotic) as unique_antibiotics,
      SUM(is_iv) as iv_events,
      ROUND(SUM(is_iv) * 100.0 / COUNT(*), 1) as pct_iv
  FROM abx_base
")

cat("\nBasic antibiotic statistics:\n")
print(abx_stats)

# Top antibiotics by route
top_abx <- dbGetQuery(con, "
  SELECT 
      antibiotic,
      route,
      COUNT(*) as events,
...
```{r}
#| label: step20-2-calculate-times

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.2: Calculate effective duration (24-hour rule)\n")
cat("=", rep("=", 50), "\n", sep = "")

dbExecute(con, "
  CREATE OR REPLACE TABLE abx_with_times AS
  ...
  WITH add_intime AS (
      SELECT 
          ...
          CAST(
              FLOOR(EXTRACT(EPOCH FROM (a.stoptime - a.starttime)) / 3600) AS INTEGER
          ) AS actual_duration_hours,
          -- 24-hour rule: minimum 24h from start, extend to stoptime if longer
          GREATEST(
              24,
              CAST(
                  FLOOR(EXTRACT(EPOCH FROM (a.stoptime - a.starttime)) / 3600) AS INTEGER
              )
          ) AS effective_duration_hours,
          -- Hours from ICU admission to antibiotic start
          CAST(
              FLOOR(EXTRACT(EPOCH FROM (a.starttime - i.intime)) / 3600) AS INTEGER
          ) AS hr_abx_start
      FROM abx_base a
      LEFT JOIN icu_stays_base i
  ...
  SELECT * FROM add_intime
  WHERE hr_abx_start >= 0  -- Only post-ICU-admission administrations
")

cat("  done\n")

# Compare actual vs effective duration
duration_comparison <- dbGetQuery(con, "
  SELECT 
      COUNT(*) as total_events,
      ROUND(AVG(actual_duration_hours), 1) as avg_actual_duration,
      ROUND(AVG(effective_duration_hours), 1) as avg_effective_duration,
      SUM(CASE WHEN actual_duration_hours < 24 THEN 1 ELSE 0 END) as events_extended_to_24h,
      ROUND(
          SUM(CASE WHEN actual_duration_hours < 24 THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 
          1
      ) as pct_extended
  FROM abx_with_times
")

cat("\nActual vs effective duration (24-hour rule):\n")
print(duration_comparison)

# Effective duration distribution
eff_dur_dist <- dbGetQuery(con, "
  SELECT 
      CASE 
          WHEN effective_duration_hours = 24 THEN '24h (minimum)'
          WHEN effective_duration_hours <= 48 THEN '25-48h'
          WHEN effective_duration_hours <= 72 THEN '49-72h'
          WHEN effective_duration_hours <= 168 THEN '73-168h (1 week)'
          ELSE '>168h (>1 week)'
  ...
  ORDER BY MIN(effective_duration_hours)
")

cat("\nEffective duration distribution (after 24-hour rule):\n")
print(eff_dur_dist)
```

## Step 21.3: Expand to Hourly Timeline

```{r}
#| label: step20-3-expand-to-hourly

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.3: Expand administration periods to hourly\n")
cat("=", rep("=", 50), "\n", sep = "")

dbExecute(con, "
  CREATE OR REPLACE TABLE abx_expanded AS
  ...
  SELECT
      ...
      route,
      is_iv,
      -- Generate hr from start for effective_duration hours
      hr_abx_start + unnest(range(0, effective_duration_hours + 1)) AS hr
  FROM abx_with_times
  WHERE effective_duration_hours >= 0
  ORDER BY stay_id, hr
")
...
## Step 21.4: Create Hourly Binary Flags
```{r}
#| label: step20-4-create-hourly-flags

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.4: Create antibiotic hourly binary flags\n")
cat("=", rep("=", 50), "\n", sep = "")

dbExecute(con, "
  CREATE OR REPLACE TABLE antibiotic_hourly AS
  SELECT 
      stay_id,
      hr,
      -- Any antibiotic in use
      1 AS antibiotic_use,
      -- IV antibiotic in use
      MAX(is_iv) AS antibiotic_iv,
      -- Number of concurrent antibiotics
...
# Hourly statistics
abx_hourly_stats <- dbGetQuery(con, "
  SELECT 
      COUNT(*) as total_hours_with_abx,
      COUNT(DISTINCT stay_id) as patients_with_abx,
      SUM(antibiotic_iv) as hours_with_iv,
      ROUND(AVG(antibiotic_count), 2) as avg_concurrent_abx,
      MAX(antibiotic_count) as max_concurrent_abx
  FROM antibiotic_hourly
")

cat("\nAntibiotic hourly statistics:\n")
print(abx_hourly_stats)

# Concurrent antibiotic count distribution
concurrent_dist <- dbGetQuery(con, "
  SELECT 
      antibiotic_count,
      COUNT(*) as hours,
      COUNT(DISTINCT stay_id) as patients
...
cat("\nConcurrent antibiotic count distribution:\n")
print(concurrent_dist)
```


## Step 21.5: Join to Person-Hourly Data

```{r}
#| label: step20-5-join-to-base

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.5: Join to person-hourly data\n")
cat("=", rep("=", 50), "\n", sep = "")

# Join antibiotic data to person_hourly_with_crrt
dbExecute(con, "
  CREATE OR REPLACE TABLE person_hourly_with_abx AS
  SELECT
      p.*,
      COALESCE(a.antibiotic_use, 0) AS antibiotic_use,
      COALESCE(a.antibiotic_iv, 0) AS antibiotic_iv,
      COALESCE(a.antibiotic_count, 0) AS antibiotic_count
  FROM person_hourly_with_gcs p
  LEFT JOIN antibiotic_hourly a
      ON p.stay_id = a.stay_id
      AND p.hr = a.hr
  ORDER BY p.stay_id, p.hr
")

cat("  created person_hourly_with_abx\n")
cat("  Added variables: antibiotic_use, antibiotic_iv, antibiotic_count\n")

# Post-join statistics
join_stats <- dbGetQuery(con, "
  SELECT 
      COUNT(*) as total_observations,
      COUNT(DISTINCT stay_id) as unique_patients,
      SUM(antibiotic_use) as hours_with_abx,
      ROUND(SUM(antibiotic_use) * 100.0 / COUNT(*), 2) as pct_hours_with_abx,
      SUM(antibiotic_iv) as hours_with_iv_abx,
      ROUND(AVG(antibiotic_count), 3) as avg_abx_count
  FROM person_hourly_with_abx
")
...
```{r}
#| label: step20-6-validation

cat("\n")
cat("=", rep("=", 50), "\n", sep = "")
cat("Step 20.6: Validation\n")
cat("=", rep("=", 50), "\n", sep = "")

# 1. Row count preservation check
row_check <- dbGetQuery(con, "
  SELECT 
      'person_hourly_with_crrt' as table_name,
      COUNT(*) as row_count
  FROM person_hourly_with_crrt
  UNION ALL
  SELECT 
      'person_hourly_with_abx',
      COUNT(*)
  FROM person_hourly_with_abx
")

cat("Row count preservation check:\n")
print(row_check)

# 2. Patient-level antibiotic use rate
patient_abx_rate <- dbGetQuery(con, "
  SELECT 
      COUNT(DISTINCT stay_id) as total_patients,
      COUNT(DISTINCT CASE WHEN antibiotic_use = 1 THEN stay_id END) as patients_with_abx,
      ROUND(
          COUNT(DISTINCT CASE WHEN antibiotic_use = 1 THEN stay_id END) * 100.0 / 
          COUNT(DISTINCT stay_id), 
          2
      ) as pct_patients_with_abx
  FROM person_hourly_with_abx
")

cat("\nPatient-level antibiotic use rate:\n")
print(patient_abx_rate)

# 3. Antibiotic use during HFNC (time >= 0)
abx_during_hfnc <- dbGetQuery(con, "
  SELECT 
      'Any antibiotic' as variable,
      SUM(CASE WHEN time >= 0 AND antibiotic_use = 1 THEN 1 ELSE 0 END) as hours_during_hfnc,
      COUNT(DISTINCT CASE WHEN time >= 0 AND antibiotic_use = 1 THEN stay_id END) as patients
  FROM person_hourly_with_abx
  
  UNION ALL
  SELECT 'IV antibiotic',
         SUM(CASE WHEN time >= 0 AND antibiotic_iv = 1 THEN 1 ELSE 0 END),
         COUNT(DISTINCT CASE WHEN time >= 0 AND antibiotic_iv = 1 THEN stay_id END)
  FROM person_hourly_with_abx
  
  ORDER BY hours_during_hfnc DESC
")

cat("\nAntibiotic use during HFNC:\n")
print(abx_during_hfnc)

# 4. Sample patient timeline
sample_abx <- dbGetQuery(con, "
  SELECT 
      stay_id, hr, time,
      antibiotic_use, antibiotic_iv, antibiotic_count,
      vasopressor, highflow
  FROM person_hourly_with_abx
  WHERE stay_id = (
      SELECT stay_id 
      FROM person_hourly_with_abx 
      WHERE antibiotic_use = 1 AND antibiotic_count >= 2
      LIMIT 1
  )
  AND hr <= 72
  ORDER BY hr
...
cat("\nSample patient antibiotic timeline (first 72 hours):\n")
print(head(sample_abx, 30))
```
